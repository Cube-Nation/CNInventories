...
@Description("command.inventory.zone.remove.desc")
@SubCommand({"zone"})
@SubCommand({"remove", "delete"})
@Permission(Name = "inventory.zone.remove", Role = CommandRole.ADMIN)
@IngameCommand
public class InventoryZoneRemoveCommand extends Command {

    public InventoryZoneRemoveCommand(BasePlugin plugin, CommandManager commandManager) {
